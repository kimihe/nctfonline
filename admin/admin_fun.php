<?php
	...
	function delete_question($question_id){
		global $dbc;
		$query = 'DELETE FROM nctf_questions WHERE question_id ='.$_GET['delid'];
        $result = $dbc->query($query);
    }

	function delete_user($user_id){
		global $dbc;
		$query = 'DELETE FROM nctf_accounts WHERE user_id ='.$user_id;
		$result = $dbc->query($query);
		$dbc->close();
	}
	...
	function delete_notice($delid){
		global $dbc;
		$query = 'DELETE FROM nctf_notice WHERE id ='.$delid;
		$result = $dbc->query($query);
		$dbc->close();
	}
?>