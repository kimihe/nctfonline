			...
			<?php
				...
				if ($result->num_rows > 0) {
					...
					while($row = $result->fetch_assoc()) {
						...
						$sOutput .= '<td>   <button type="button" class="btn btn-primary btn-xs"><a href="javascript:if(confirm(\'确实要删除吗?\'))location=\'admin_question.php?delid='.$row["question_id"].'\'">Del</button>'.' <td>';
						$sOutput .'</tr>';					
					}
				}else{
